...
def plot_survival_curve(
    ...
    legend_loc: str = "lower left", # Changed default
    colors: Optional[List[str]] = None,
    linestyles: Optional[List[str]] = None,
    high_contrast: bool = False, # Added parameter
    ...
    high_contrast : bool, default=False
        If True, uses a high-contrast style for the plot.
    ...
    # Apply high-contrast style if requested
    style_context = plt.style.context('seaborn-v0_8-darkgrid' if high_contrast else 'default')
    ...
    with style_context:
        ...
        if ax is None:
            ...
            fig = ax.figure
            # If ax is provided, risk_ax needs to be handled differently or assumed not used
            risk_ax = None # Assume risk_table not used if ax is provided

        # Default colors and linestyles
        num_curves_to_plot = 1 if survival_curves.ndim == 1 else len(survival_curves)
        if colors is None:
            # Use a colorblind-friendly palette like viridis
            colors = plt.cm.viridis(np.linspace(0, 1, max(10, num_curves_to_plot)))
        ...
        if risk_table and risk_ax: # Check if risk_ax exists
            # For single curve
            if survival_curves.ndim == 1:
    ...
    return fig # Return the figure outside the context manager
...
def plot_cumulative_incidence(
    ...
    legend_loc: str = "upper left", # Changed default (upper left often better for CIF)
    colors: Optional[List[str]] = None,
    high_contrast: bool = False, # Added parameter
    ax: Optional[plt.Axes] = None
) -> Figure:
    ...
    with style_context:
        ...
        # Default colors
        num_risks_to_plot = cif.shape[0] if cif.ndim == 2 else cif.shape[1]
        if colors is None:
            # Use a colorblind-friendly palette like viridis
            colors = plt.cm.viridis(np.linspace(0, 1, max(10, num_risks_to_plot)))
...
def plot_calibration_curve(
    ...
    with style_context:
        ...
        # Define a colorblind-friendly color for the Lowess curve
        lowess_color = plt.cm.viridis(0.7) # Use a color from the viridis palette

        # Add lowess curve if available
        try:
            from statsmodels.nonparametric.smoothers_lowess import lowess
            y_smooth = lowess(mean_observed, mean_predicted, frac=0.6)
            ax.plot(y_smooth[:, 0], y_smooth[:, 1], color=lowess_color, linestyle='-', lw=2, label='Lowess curve')
        except ImportError:
            # Fallback to simple interpolation
            ax.plot(mean_predicted, mean_observed, color=lowess_color, linestyle='-', lw=2, label='Interpolation')
...
def plot_risk_stratification(
    ...
    with style_context:
        ...
        # Default colors (use colorblind-friendly palette)
        if colors is None:
